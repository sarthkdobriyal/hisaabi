...
/* Dark mode is OS-driven (prefers-color-scheme), matching the app's existing
   behavior — no .dark class toggle. This also keeps all `dark:` utilities and
   the shadcn tokens flipping together. */
...
@theme inline {
  ...
  --animate-fade-up: fade-up 0.2s ease-out both;
  --animate-scale-in: scale-in 0.15s ease-out both;
  --animate-pulse-dot: pulse-dot 1.2s ease-in-out infinite;
}
...
.brand-gradient {
  background-image: linear-gradient(135deg, var(--brand) 0%, var(--brand-600) 100%);
}
...
/* Motion — message entrance, confirm-card pop, typing dots */
@keyframes fade-up {
  from {
    opacity: 0;
